...
test('handle mouse action',async({page})=>{
    await page.goto('https://testautomationpractice.blogspot.com/')

    const loc = page.locator('.dropbtn')
    await loc.hover()
...
test('handle right click',async({page})=>{
   ...
   const loc =   page.locator('.context-menu-one').first()
...
test('Double click', async({page})=>{
 await page.goto("https://testautomationpractice.blogspot.com/")
 const butto = page.locator("[ondblclick='myFunction1()']")
  await butto.dblclick()
