...
test('Playwright Assertion Demo', async({page})=>{
    ...
    await page.goto('https://demowebshop.tricentis.com/');
     ...
     // 1. Auto-retrying assertion (automatically retries until it passes or times out)
      ...
      // Auto-retry: waits for the element to be visible and have expected text
      ...
      expect(welcometext).toContainText('Welcome') // non-retrying

      //3. Negative Matcher (Applicable for both auto-retrying & Non-retrying assertion)

      //await expect(page.locator('text=Welcome to our store')).not.toBeVisible()
     // expect(page.locator('text=Welcome to our store')).not.toContainText('Welcome')
})
