...
test('static table',async({page})=>{
    await page.goto('https://testautomationpractice.blogspot.com/')
      ...
      //  const column : Locator = page.locator("table[name='BookTable'] tr th")
      ...
      // 3) Read all data from 2nd row (index 2 means 3rd row including header)
        ...
        //  await expect(secondRowCells).toHaveText([ 'Learn Java', 'Mukesh', 'Java', '500' ])
         ...
         for(let rowData of rowArr.slice(1)){
         ...
         for(let row of rowArr.slice(1))
         {
             ...
             if(authorName === 'Mukesh'){
                console.log(`${authorName} and ${bookName}`);
         ...
         for(let row of rowArr.slice(1)){
            ...
            let price = await row.locator('td').nth(3).innerText()
